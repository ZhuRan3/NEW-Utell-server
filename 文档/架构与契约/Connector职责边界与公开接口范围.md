...
> **状态：草案(pending),2026-08-27 起草;2026-08-28 起暂缓推进(Connector Owner 未定,Q-PH/SV-2026-041),已确认的 #3/#4 契约层结论继续有效**
...
> 确认口径：本文只**归并**已确认语义，不设默认值；凡"待确认"条目须按 `开发规则规范/01-需求对齐与Q&A规范.md` 一次一问走 Q&A 后回写。Owner:暂缓未定(2026-08-28 用户决定 Connector 工程暂缓,Q-PH/SV-2026-041,回滚 Q-038);本文保留为未来 Connector 工程的需求基线,清单 #3/#4 结论属契约层语义继续有效,#1 重开、#2/#5–#9 待新 Owner 明确后推进。
...
- 每会话每方向单调 `uint64` 序号,拒绝重复/回退;新会话序号重置;正式封套为 plaintext=[8 字节大端序号][应用载荷] 整体经 Noise 加密、严格单调窗口=0;封套重放/乱序/篡改/过期不新增公开码,端点静默丢弃并记本地脱敏诊断(已确认,Q-PH/SV-2026-039)。
...
| # | 待确认项 | 当前口径 | 建议 Q&A |
| --- | --- | --- | --- |
| 1 | Connector Owner 归属 | **暂缓未定**(2026-08-28,Q-PH/SV-2026-041 回滚 Q-038):Connector 工程暂缓 | 重开 |
| 2 | Noise 固定向量与失败语义的共同签收 | 单向已完成 Spike;签收主体待新 Owner 明确(Q-PH/SV-2026-041) | 暂缓 |
...
| 2026-08-28 | v0.5 | 清单 #1 重开:用户决定 Connector 工程暂缓、Owner 未定(Q-PH/SV-2026-041,回滚 Q-038);#3/#4 结论保留,#2/#5–#9 暂缓;#5 部分作答作废未登记。 | Zhu3xx |
