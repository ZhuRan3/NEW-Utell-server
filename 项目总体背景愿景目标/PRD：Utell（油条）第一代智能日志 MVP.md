...
最近 30 秒是 P0 已确认参数，变更时必须同步测试用例与状态文案。
...
- 正常操作流程：Connector 每 10 秒报告能力和权威库健康状态；手机按 5\.2 的状态定义显示唯一主状态与具体断点。每次进入 READY 后，手机按 5\.5 发起 Projection 同步；恢复连接后只恢复健康检查、查询当前会话内未知结果和刷新 Projection。
...
- 正常操作流程：手机只渲染本次前台会话从 Connector 获取的最新 Projection；卡片按手机当前时区日期分组，展示标题、摘要、状态、原文入口、展开、确认和纠正入口。用户打开原文入口时，手机在线按当前卡片的 `event_id` 向 Connector 请求原文，并仅在当前前台会话内存中展示。

- 异常/边界情况：无数据时展示空状态；非 READY 时展示链路断点，不得读取或展示手机持久化业务缓存；原文请求失败、断线或非 READY 时不展示原文、不写入缓存且不得自动重试；时间未知时进入“时间待确认”分组；App 重启或恢复后必须重新获取最新 Projection。
...
- AC\-029：Given 手机处于 READY 且当前会话持有 event\_id 对应的最新 Projection；When 用户打开原文入口；Then 手机在线向 Connector 请求对应 Capture 原文，仅在当前会话内存中展示；请求失败、断线或 App 退出后不得保留、自动重试或恢复原文。
...
- 正常操作流程：Relay 处理身份、配对、公钥登记、密文路由、ACK、重连、pairing 级限流、全局握手并发/速率保护和最小脱敏审计；全局保护的单实例初始基线为最多 `100` 个并发握手、每 `60 秒`最多 `300` 次握手尝试。进程重启后，已配对端点可重新认证并恢复健康检查。

- 异常/边界情况：Relay 不建立离线业务队列；未即时投递的密文必须丢弃并向发件端报告；不得保存可解密业务正文；pairing 级或全局握手保护超限、认证失败均返回稳定脱敏错误码 `RATE_LIMITED` 或对应认证错误。全局初始基线须经容量 Spike 校准。
...
|Card Projection|event\_id、event\_version、展示字段、投影时间|经端到端加密发送，手机仅前台内存使用；不包含 Capture 原文，原文由独立在线请求按需获取。|
...
- 脱敏审计元数据最长保留 14 天（2026-08-23 用户确认）；到期自动删除。
...
|---|---|---|---|
|指标阈值|接收、解析、投影指标阈值已确认（v0.4）；实测复核随试运行|待指定|试运行|
...
|隐私合规|Relay 元数据 14 天保留（已确认）与诊断导出|待指定|发布前|
...
* [ ] FR\-001 至 FR\-009 的 AC\-001 至 AC\-029 全部通过。
...
|2026\-08\-23|v0\.3|Relay 脱敏审计元数据保留期由 30 天建议值确认为 14 天（两端台账 C2）。|Zhu3xx|
|2026\-08\-23|v0\.4|P0 成功指标阈值确认为建议值（两端台账 E1）；第一代不设账号体系，设备配对即身份。|Zhu3xx|
|2026\-08\-23|v0\.5|确认原文追溯保留在 P0：在线按 event\_id 向 Connector 按需读取，仅当前会话内存展示，不持久化、不自动重试；同步 10s/30s 参数状态。|Zhu3xx|
